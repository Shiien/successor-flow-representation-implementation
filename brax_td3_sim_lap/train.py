...
def _init_training_state(
    ...
    feature_params = td3_network.feature_network.init(key_feature)
    feature_optimizer_state = q_optimizer.init(feature_params)
    task_params = td3_network.task_network.init(key_task)
...
def train(
    ...
    critic_loss, actor_loss, task_loss, lap_loss = make_losses(
    ...
    task_update = (
        gradients.gradient_update_fn(                                                  
            task_loss, q_optimizer, pmap_axis_name=_PMAP_AXIS_NAME
        )
    )
    lap_update = (
        gradients.gradient_update_fn(                                                  
    ...
    def sgd_step(
        ...
        key, key_critic, key_task, key_lap = jax.random.split(key, 4)
        ...
        update_task = training_state.gradient_steps.lo % 10 == 0
        (
            task_loss,
            task_params,
            task_optimizer_state,
    ...
    def training_step(
        ...
        task = td3_network.task_network.apply(training_state.task_params)
